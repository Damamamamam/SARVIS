...
const FLAGGED = ['com.instagram.android'];
...
test('nudge fires once when a flagged app crosses the threshold', () => {
  ...
  g.observe('com.instagram.android', 'Instagram', 30_000);
  const ev = g.observe('com.instagram.android', 'Instagram', 30_000);
  assert.ok(ev);
  assert.equal(ev!.app, 'Instagram');
...
test('nudge does not fire twice for the same app', () => {
  ...
  let count = 0;
  g.onNudge(() => count++);
  g.observe('com.instagram.android', 'Instagram', 1200);
  g.observe('com.instagram.android', 'Instagram', 5000);
...
test('non-flagged apps never trigger a nudge', () => {
  ...
  g.observe('com.google.android.dialer', 'Phone', 999999);
...
test('snapshot tracks per-app daily totals', () => {
  ...
  g.observe('com.instagram.android', 'Instagram', 1500);
  g.observe('com.twitter.android', 'Twitter', 800);
  const snap = g.snapshot();
  assert.equal(snap.length, 2);
  const ig = snap.find((s) => s.app === 'com.instagram.android');
  assert.equal(ig?.dailyTotalMs, 1500);
});
